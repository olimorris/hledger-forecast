...
RSpec.describe HledgerForecast::Settings do
  def settings_rows_from(csv)
    CSV.parse(csv, headers: true, header_converters: ->(h) { h.to_s.tr('-', '_').to_sym }, converters: :numeric)
       .select { |r| r[:type] == 'settings' }
  end

  describe 'defaults when no settings rows are present' do
    let(:rows) { settings_rows_from("type,frequency,account,from,to,description,category,amount,roll_up,summary_exclude\n") }
